...
class HierarchicalClassifierTestCase(unittest.TestCase):
    ...
    def test_init_2(self):
        ...
        clf = MockClassifier()
        inst = base.HierarchicalClassifier(
            binary_clf=clf, n_jobs=2, clf_params={"C": 10.0}
        )
...
def mock_split(obj):
    ...
    exp_weight = (
        1.0 / 2.0 - 1.0 / 4.0 * math.sqrt(1.0 / 3.0) - 1.0 / 4.0 * 1.0 / 3.0
    )
    if len(obj.elements) == 3:
        ...
        G_pos = nx.Graph()
        G_pos.add_node(2)
    obj.negative_child_ = base.GraphClassifier(
        binary_clf=MockClassifier, graph=G_neg
    )
    obj.positive_child_ = base.GraphClassifier(
        binary_clf=MockClassifier, graph=G_pos
    )
